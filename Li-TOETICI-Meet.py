...
def generateImage():
    ...
    Label = np.reshape(Label, (176*numOfImage, 1)).flatten()
...
def train(BATCH_SIZE):
    ...
    originalCSI, labelplus = generateImage()    # 原始振幅数据集与标签(176*10, 256, 256, 3)，(176*10, 1)
    data = (np.array(originalCSI, dtype='float'))
    labels = np.array(labelplus)
    lb = LabelBinarizer()
    phaseLabel = lb.fit_transform(labels)

    traindata, testdata, trainlabel, testlabel = train_test_split(data, phaseLabel, test_size=0.2, random_state=9)
    ...
    # d.save(filepath=r'/Users/zhuxiaoqiang/Desktop/IEEE Trans/Third paper/代码/third-code/CompareExperiments/model-TOETICI-Meet-v1.h5')

    Mymodel = load_model(filepath=r'/Users/zhuxiaoqiang/Desktop/IEEE Trans/Third paper/代码/third-code/CompareExperiments/model-TOETICI-Meet-v1.h5')
    ...
    print('test time is ', round(Test_time, 6), 'seconds')  # Meet  28.811148 seconds
    accuracyLab(predictOfPhaseImage, label)
    # saveTestErrorMat(predictOfPhaseImage, label, 'Li-TOETICI-Meet-Error')
...
if __name__ == "__main__":
    ...
    # 4.075876134087604     random 9best