...
def generatePhaseImage():   #已完成，不再重新生成图片
    ...
    for i in range(16):
        for j in range(11):
            filePath = r'/Users/zhuxiaoqiang/Downloads/thirdDataSet/55imaginary_part/imaginary' + xLabel[i] + yLabel[j] + '.mat'
            if (os.path.isfile(filePath)):
                ...
                for k in range(50):
                    ...
                    saveName = r'/Users/zhuxiaoqiang/Downloads/thirdDataSet/55imaginary_part/image-phase-3D/Phase' + xLabel[i] + yLabel[j] +"-"+midString[k]+ ".jpg"
...
def phaseMatrix():
    ...
    for i in range(16):
        for j in range(11):
            for k in range(numOfImage):
                filePath = r'/Users/zhuxiaoqiang/Downloads/thirdDataSet/55imaginary_part/image-phase-3D/Phase' + xLabel[i] + yLabel[
                           j] + "-" + midString[k] + ".jpg"
                if (os.path.isfile(filePath)):
    ...
    phaseLabel = np.reshape(phaseLabel, (176*numOfImage, 1)).flatten()
...
def constructCNN(traindata, testdata, trainlabel, testlabel):
    ...
    # model.save(filepath=r'D:\pythonWork\indoor Location\third-code\model-meeting-50images.h5')
    # model.save_weights('model_weights-meeting-v1.h5')
    with open('logMeet.txt', 'wb') as file_txt:
...
def main():
    ...
    np.save("55train", trainRaw)
    np.save("55test", testRaw)
    np.save("55trainLabel", trainRawlabel)
    np.save("55testLabel", testRawlabel)
    ...
    # print('Retraining time is ', round(Retraining_time, 6), 'seconds')   #  444.167911 s

    'cluster the predicts of phase images for every position'
    from sklearn.cluster import KMeans
    Mymodel = load_model(r'/Users/zhuxiaoqiang/Desktop/IEEE Trans/Third paper/代码/third-code/model-meeting-v1.h5')
    ...
    predictOfPhaseImage= np.reshape(cluster,(176,2))

    # weights = np.array(Mymodel.get_weights())
    # # Mymodel.evaluate(testdata,testlabel)

    'BLS classification regression'
    N1 = 30  # # of nodes belong to each window
    N2 = 5  # # of windows -------Feature mapping layer
    N3 = 50  # # of enhancement nodes -----Enhance layer
    L = 10  # # of incremental steps   9 retrain 1.279177819487699 m
    ...
    print('test time is ', round(Test_time, 6), 'seconds')  # Meet  0.000907 seconds
    accuracyMeeting(list7, label)   #   1.2853135206951467 m
    # saveTestErrorMat(list7, label, 'ILCL-Meet-Error')
...
def accuracyMeeting(label1, label2):
    error = np.asarray(label1 - label2)
    print(np.mean(np.sqrt(error[:, 0] ** 2 + error[:, 1] ** 2)) * 60 / 100 , 'm')
